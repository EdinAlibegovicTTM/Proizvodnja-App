...
export function DoradaList({ refresh, onEdit }: { refresh?: number; onEdit?: (proces: any) => void }) {
  ...
  const handleDelete = async (id: string) => {
    if (!window.confirm("Da li ste sigurni da želite obrisati ovaj proces dorade?")) return
    const { error } = await supabase.from("dorada").delete().eq("id", id)
    if (!error) setLocalRefresh(r => r + 1)
    else alert("Greška pri brisanju procesa")
  }

  if (loading) return <div>Učitavanje...</div>
  if (error) return <div className="text-red-500">{error}</div>
  if (!procesi.length) return <div>Nema procesa dorade.</div>
  ...
  return (
    <div className="space-y-4">
      ...
      {procesi.map((proces) => (
        <div key={proces.id} className="border rounded p-4 flex flex-col md:flex-row md:items-center md:justify-between">
          <div>
            <div className="font-bold">{proces.novi_paket}</div>
            <div className="text-sm text-muted-foreground">{proces.masina}</div>
            <div className="text-xs">{proces.datum?.slice(0, 10)}</div>
            <div className="text-xs">Radnik: {proces.radnik}</div>
          </div>
          <div className="mt-2 md:mt-0 flex items-center gap-2">
            <button onClick={() => handleDelete(proces.id)} className="ml-4 text-red-600 hover:underline text-sm">Obriši</button>
            <button onClick={() => onEdit && onEdit(proces)} className="ml-2 text-blue-600 hover:underline text-sm">Uredi</button>
          </div>
        </div>
      ))}
    </div>
